...
1. The file is always in blocks - i.e into the buffer, whether we are reading file for obtaining next token or removing comments.
...
char* tokenMap[] = {"ASSIGNOP","COMMENT","FUNID","NUM","RNUM","STR","END","INT","STRING","MATRIX","MAIN","OP","CL","COMMA","IF","ELSE","ENDIF","READ","PRINT","FUNCTION","NOT","LT","LE","EQ","GT","GE","NE","$","ERROR",  "LABEL", "GOTO","DECLARE","ID", "COLON","LIST","OF", "VARIABLES","INTEGER","SC",
...
Token* getNextToken(FILE** file_ptr){
	...
	while(1){
		switch(state){
			...
			case 12:{
				strcpy(lexeme,":");
				addToken(token, COLON, lexeme, -1, NULL);	
				final_state_reached = 1;
			}break;
			...
			case 14:{
				strcpy(lexeme,"{");
				addToken(token, LCB, lexeme, -1, NULL);	
				final_state_reached = 1;
			}break;
			...
			case 17:{
				strcpy(lexeme,"}");
				addToken(token, RCB, lexeme, -1, NULL);
				final_state_reached = 1;
			}break;
			...
			case 42:{
				lexeme[lexeme_ptr] = '\0';
				addToken(token, DOT, lexeme, -1, NULL);	
				final_state_reached = 1;
			}break;
